...
double_tensor MLPClassifier::predict(DataLoader<double, double>* pLoader,
  ...
  for (auto batch : *pLoader) {
    ...
    auto data = batch.getData();
        ...
        if (first_batch) {
            results = xt::zeros<double>({total_batch * data.shape()[0], output.shape()[1]});
            first_batch = false;
        }

        auto batch_start = nsamples;
        auto batch_end = nsamples + output.shape()[0];
        xt::view(results, xt::range(batch_start, batch_end), xt::all()) = output;
