...
xt::xarray<double> FCLayer::forward(xt::xarray<double> X) {
    ...
    xt::xarray<double> m_T = xt::transpose(m_aWeights);
    auto Y = xt::linalg::dot(X, m_T); 
    if (m_bUse_Bias) {
        Y += m_aBias;
    }
    return Y;
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY) {
    ...
    xt::xarray<double> dW_batch = xt::zeros<double>({DY.shape()[0], m_aWeights.shape()[0], m_aWeights.shape()[1]});
    if (m_bUse_Bias) {
        m_aGrad_b = xt::sum(DY, {0});
    }
    for (std::size_t i = 0; i < DY.shape()[0]; ++i) {
        xt::xarray<double> dy = xt::view(DY, i);  
        xt::xarray<double> x = xt::view(m_aCached_X, i);  
        xt::view(dW_batch, i) = xt::linalg::outer(dy, x);  
    }
